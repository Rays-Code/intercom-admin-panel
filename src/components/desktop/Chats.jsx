...
const Chats = () => {
  ...
  // Getting the selected user from the userContext
  const selectedUser = user.find(u => u.id === userNumber);

  if (!selectedUser) return <div>No chat found</div>;

  // Safe check for chats
  if (!Array.isArray(selectedUser.chats)) {
    return <div>Error: Chat history is not available.</div>;
  }
            ...
            {/* Mapping through toUser */}
             {chat.toUser.map((msg, i) => {
                return <div className='flex items-center ml-10'>
                        ...
                        <div key={`toUser-${i}`} className='px-5 pt-3 pb-4 bg-[#eeefec] rounded-xl mr-50 m-3 min-w-xs min-h-[72px] keep-all w-fit'>
             ...
             {chat.fromUser.map((msg, i) => {
                return <div className='flex items-center mr-10'>
                    <div key={`from-${i}`} className='flex flex-col pl-5 pt-3 pb-2 pr-3 bg-[#c7d4f5] rounded-xl ml-102 m-3 max-w-md min-w-xs keep-all w-fit'>
                      <div className='text-sm'>{msg}</div>
